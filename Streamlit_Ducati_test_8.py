...
if df_saturazione.max(axis=1)[0]>1: # fake, solo per verificare la condizione
    ...
    vincoli_produzione_new = st.experimental_data_editor(vincoli_produzione)
    df_confronto = pd.concat([vincoli_produzione_new,df_fabbisogno],axis=0)
    saturazione = []
    for i in range(len(df_confronto.columns)):
        sat = (df_confronto.iloc[1][i]/df_confronto.iloc[0][i])
        saturazione.append(sat)
...
#st.write('Vincoli di produzione', vincoli_produzione)
